...
function Assert-OpenEvolveKey {
    ...
    if (-not $key -or -not $key.Trim()) {
        throw "openai_api_key_missing: set OPENAI_API_KEY in benchmarks/tracks/openevolve/.env.local"
    }
    if ($key -match 'your_key_here|<provider-key>|<.*>|paste') {
        throw "openai_api_key_placeholder: replace the placeholder in benchmarks/tracks/openevolve/.env.local"
    }
}
...
$repo = Get-RepoRoot
if (-not $EnvFile) {
    $EnvFile = Join-Path $repo "benchmarks\tracks\openevolve\.env.local"
}
...
$target = Join-Path $repo "benchmarks\tracks\openevolve\targets\candidate_spool_policy.py"
$evaluator = Join-Path $repo "benchmarks\tracks\openevolve\evaluators\evaluate_candidate_spool_policy.py"
$config = Join-Path $repo "benchmarks\tracks\openevolve\configs\candidate_spool_policy_smoke.yaml"
...
if (-not $OutputDir) {
    ...
    $OutputDir = Join-Path $repo "benchmarks\tracks\openevolve\workspaces\runs\candidate_spool_policy_$runId"
}
