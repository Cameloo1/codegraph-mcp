...
- Verdict: **fail**
- Reason: failed targets: bytes_per_proof_edge, proof_db_mib_stretch
...
| Field | Value |
| --- | --- |
| `artifact_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_proof_build_fix_20260513.sqlite |
| `artifact_metadata_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_proof_build_fix_20260513.artifact.json |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `cold_proof_build_total_wall_ms` | 60000 | 59414 | pass | Cold proof build intended target is <=60 seconds. |
| `cold_proof_build_total_profile_ms` | reported | 59414 | pass | Profile total from compact proof baseline. |
...
| `db_write_time_ms` | reported | 35198 | pass | Known dominant cold-build stage. |
...
| `integrity_check_time_ms` | reported | 646.617 | pass | Cold proof DB integrity-check duration. |
...
| --- | ---: | ---: | --- | --- | --- |
| `proof-build-only` | 59414 | 0.990 | pass | true | actual production proof-mode cold build as persisted by the compact gate |
| `proof-build-plus-validation` | 60061 | 1.001 | fail | false | proof build plus the separately persisted integrity_check duration |
| `proof-build-plus-audit` | 3559779 | 59.330 | reported | false | sequential proof build plus separate audit-sidecar build |
| `proof-build-plus-audit-plus-validation` | 3560426 | 59.340 | reported | false | operator gate bundle, not the production proof-build-only target |
...
| --- | ---: | ---: | --- | --- | --- |
| `actual_proof_db_build_total` | 59414 | 100.00% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.wall_ms` | This is the persisted cold proof build number. |
| `production_persistence_and_global_reduction_bucket` | 35198 | 59.24% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.db_write_ms` | This bucket is currently broad: it includes SQLite persistence plus post-local global reduction, PathEvidence refresh, index recreation, ANALYZE/checkpoint work, and transaction commit where applicable. |
| `source_scan_parse_extract_dedupe_reducer_residual` | 24216 | 40.76% | true | `wall_ms - db_write_ms` | The compact gate did not persist nested cold-build spans for these phases; as a combined residual they are below 5% of proof-build-only wall time. |
| `integrity_check_validation` | 646.617 | 1.08% | false | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.integrity_check_ms` | Integrity checking is separately measured and is not the dominant cause. |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `entity_name_lookup` | 250.000 | 1.006 | 1.781 | 1.781 | pass | Default entity-name lookup must compile and use compact proof entity dictionaries. |
| `symbol_lookup` | 250.000 | 118.420 | 125.540 | 125.540 | pass | Default symbol lookup must resolve object ID, name, and qualified-name paths without audit/debug sidecars. |
| `qname_lookup` | 250.000 | 10.797 | 11.166 | 11.166 | pass | Default qname lookup must use compact qualified-name reconstruction. |
| `text_fts_query` | 500.000 | 0.025 | 0.238 | 0.238 | pass | Proof-mode text query must use compact FTS or fail explicitly. |
| `relation_query_calls` | 500.000 | 149.162 | 175.895 | 175.895 | pass | Bounded CALLS relation lookup must use proof edges and compact compatibility views. |
| `relation_query_reads_writes` | 500.000 | 148.847 | 155.593 | 155.593 | pass | Bounded READS/WRITES lookup must compile against compact proof edges. |
| `path_evidence_lookup` | 500.000 | 1.430 | 2.292 | 2.292 | pass | Stored PathEvidence lookup must use proof-mode materialized lookup tables. |
| `source_snippet_batch_load` | 500.000 | 29.885 | 32.675 | 32.675 | pass | Source snippets must load from source files using proof-edge/source-span rows, not redundant SQLite snippet storage. |
| `context_pack_normal` | 2000 | 76.495 | 79.331 | 79.331 | pass | Normal context_pack must use proof-mode stored PathEvidence first and keep source-span labels. |
| `unresolved_calls_paginated` | 1000 | 50.259 | 56.198 | 56.198 | pass | Unresolved-calls pagination may read the heuristic/debug sidecar by design, but it must remain explicit and bounded. |
...
| `cold_proof_build_ms_vs_compact_baseline` | 59414 | 59414 | 0.000 | unchanged |
| `proof_db_mib_vs_compact_baseline` | 320.630 | 171.184 | -149.446 | improved |
| `proof_db_mib_vs_previous_comprehensive` | 171.215 | 171.184 | -0.031 | improved |
