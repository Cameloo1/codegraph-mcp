...
| Field | Value |
| --- | --- |
| `artifact_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_1778642765060.sqlite |
| `artifact_metadata_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_1778642765060.artifact.json |
...
| `file_edges` | table | 62886 | 6.578 | 3.84% | 6914048 | -16384 | proof_optional |
| `entities` | table | 89407 | 6.473 | 3.78% | 6787072 | 0 | proof_required |
| `file_source_spans` | table | 61932 | 6.465 | 3.78% | 6795264 | -16384 | proof_required |
| `idx_file_edges_edge` | index | unknown | 5.875 | 3.43% | 6168576 | -8192 | proof_optional |
| `qname_prefix_dict` | table | 48603 | 5.863 | 3.43% | 6148096 | 0 | dictionary |
| `idx_file_source_spans_span` | index | unknown | 5.797 | 3.39% | 6090752 | -12288 | proof_optional |
| `idx_qualified_name_parts` | index | unknown | 4.410 | 2.58% | 4624384 | 0 | dictionary |
| `path_evidence` | table | 4096 | 4.352 | 2.54% | 4587520 | -24576 | derived_cache |
...
| `edges` | table | 31775 | 1.980 | 1.16% | 2109440 | -32768 | proof_required |
| `files` | table | 4975 | 1.617 | 0.94% | 1695744 | 0 | proof_required |
| `callsite_args` | table | 27980 | 1.469 | 0.86% | 1540096 | 0 | structural |
| `path_evidence_edges` | table | 4096 | 1.273 | 0.74% | 1306624 | 28672 | derived_cache |
...
| `edge_provenance_dict` | table | 1131 | 0.109 | 0.06% | 118784 | -4096 | dictionary |
...
| `db_write_time_ms` | reported | 227728 | pass | Known dominant cold-build stage. |
...
| `integrity_check_time_ms` | reported | 1029 | pass | Cold proof DB integrity-check duration. |
...
| `proof-build-plus-validation` | 332897 | 5.548 | fail | false | proof build plus the separately persisted integrity_check duration |
| `proof-build-plus-audit` | 3832233 | 63.871 | reported | false | sequential proof build plus separate audit-sidecar build |
| `proof-build-plus-audit-plus-validation` | 3833262 | 63.888 | reported | false | operator gate bundle, not the production proof-build-only target |
...
| `production_persistence_and_global_reduction_bucket` | 227728 | 68.62% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.db_write_ms` | This bucket is currently broad: it includes SQLite persistence plus post-local global reduction, PathEvidence refresh, index recreation, ANALYZE/checkpoint work, and transaction commit where applicable. |
| `source_scan_parse_extract_dedupe_reducer_residual` | 104140 | 31.38% | true | `wall_ms - db_write_ms` | The compact gate did not persist nested cold-build spans for these phases; as a combined residual they are below 5% of proof-build-only wall time. |
| `integrity_check_validation` | 1029 | 0.31% | false | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.integrity_check_ms` | Integrity checking is separately measured and is not the dominant cause. |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `entity_name_lookup` | 250.000 | 2.300 | 2.923 | 2.923 | pass | Default entity-name lookup must compile and use compact proof entity dictionaries. |
| `symbol_lookup` | 250.000 | 209.346 | 240.703 | 240.703 | pass | Default symbol lookup must resolve object ID, name, and qualified-name paths without audit/debug sidecars. |
| `qname_lookup` | 250.000 | 20.158 | 28.465 | 28.465 | pass | Default qname lookup must use compact qualified-name reconstruction. |
| `text_fts_query` | 500.000 | 0.114 | 0.297 | 0.297 | pass | Proof-mode text query must use compact FTS or fail explicitly. |
| `relation_query_calls` | 500.000 | 243.388 | 277.416 | 277.416 | pass | Bounded CALLS relation lookup must use proof edges and compact compatibility views. |
| `relation_query_reads_writes` | 500.000 | 250.836 | 271.406 | 271.406 | pass | Bounded READS/WRITES lookup must compile against compact proof edges. |
| `path_evidence_lookup` | 500.000 | 3.821 | 5.489 | 5.489 | pass | Stored PathEvidence lookup must use proof-mode materialized lookup tables. |
| `source_snippet_batch_load` | 500.000 | 52.299 | 63.716 | 63.716 | pass | Source snippets must load from source files using proof-edge/source-span rows, not redundant SQLite snippet storage. |
| `context_pack_normal` | 2000 | 138.760 | 162.344 | 162.344 | pass | Normal context_pack must use proof-mode stored PathEvidence first and keep source-span labels. |
| `unresolved_calls_paginated` | 1000 | 95.033 | 123.782 | 123.782 | pass | Unresolved-calls pagination may read the heuristic/debug sidecar by design, but it must remain explicit and bounded. |
...
| `proof_db_mib_vs_previous_comprehensive` | 171.281 | 171.184 | -0.098 | improved |
