...
Execution mode: `explicit_artifact_reuse`. The benchmark used an explicitly supplied proof DB artifact. Storage and cold-build results are claimable only when freshness metadata matches the current schema/build.

## Section 1 - Executive Verdict

- Verdict: **fail**
- Reason: failed targets: bytes_per_proof_edge, cold_proof_build_total_wall_ms, proof_db_mib_stretch
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `proof_db_freshly_built` | true unless explicit artifact reuse requested | false | pass | Comprehensive benchmark builds a fresh proof DB by default; explicit reuse must be labeled. |
| `artifact_reuse_marked` | reported | true | pass | Explicit artifact reuse is visible in the report. |
...
| Field | Value |
| --- | --- |
| `artifact_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\audit\artifacts\minimal_storage_package_autoresearch_proof.sqlite |
| `artifact_metadata_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\audit\artifacts\minimal_storage_package_autoresearch_proof.artifact.json |
...
| `storage_mode` | proof |
| `db_size_bytes` | 179568640 |
| `integrity_status` | ok |
| `build_duration_ms` | 1941747 |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `proof_db_bytes` | reported | 179601408 | pass | Proof DB family size in bytes. |
| `proof_db_mib` | 250.000 | 171.281 | pass | Proof DB family must be <=250 MiB. |
| `proof_db_mib_stretch` | 150.000 | 171.281 | fail | Stretch target is <=150 MiB. |
...
| `total_artifact_bytes` | reported | 1243234304 | pass | Proof + audit artifact bytes for operator planning. |
| `wal_bytes` | reported | 0 | pass | WAL bytes are reported separately. |
| `table_bytes` | reported | 139509760 | pass | dbstat table bytes. |
| `index_bytes` | reported | 40026112 | pass | dbstat index bytes. |
| `bytes_per_proof_edge` | 120.000 | 5639 | fail | Whole proof DB bytes divided by physical proof-edge rows; this remains a bloat signal. |
| `bytes_per_edge_table_plus_index` | 120.000 | 119.350 | pass | Physical edge table plus edge indexes per proof edge. |
...
| `bytes_per_path_evidence_row` | reported | 1120 | pass | Average total bytes per PathEvidence row. |
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| `template_entities` | table | 653819 | 55.453 | 32.38% | 87523328 | -29376512 | template |
| `template_edges` | table | 169290 | 16.922 | 9.88% | 40804352 | -23060480 | template |
...
| `symbol_dict` | table | 176558 | 7.703 | 4.50% | 30392320 | -22315008 | dictionary |
| `file_edges` | table | 62959 | 6.594 | 3.85% | 6914048 | 0 | proof_optional |
| `file_source_spans` | table | 62005 | 6.480 | 3.78% | 6795264 | 0 | proof_required |
| `entities` | table | 89407 | 6.473 | 3.78% | 6803456 | -16384 | proof_required |
| `idx_file_edges_edge` | index | unknown | 5.883 | 3.43% | 6168576 | 0 | proof_optional |
| `qname_prefix_dict` | table | 48603 | 5.863 | 3.42% | 22274048 | -16125952 | dictionary |
| `idx_file_source_spans_span` | index | unknown | 5.809 | 3.39% | 6094848 | -4096 | proof_optional |
| `idx_qualified_name_parts` | index | unknown | 4.410 | 2.57% | 12742656 | -8118272 | dictionary |
| `path_evidence` | table | 4096 | 4.375 | 2.55% | 8519680 | -3932160 | derived_cache |
| `qualified_name_dict` | table | 275831 | 4.199 | 2.45% | 12173312 | -7770112 | dictionary |
| `idx_symbol_dict_hash` | index | unknown | 3.504 | 2.05% | 14602240 | -10928128 | dictionary |
| `edges` | table | 31848 | 2.012 | 1.17% | 2105344 | 4096 | proof_required |
| `files` | table | 4975 | 1.617 | 0.94% | 1695744 | 0 | proof_required |
| `callsite_args` | table | 27980 | 1.469 | 0.86% | 1540096 | 0 | structural |
| `path_evidence_edges` | table | 4096 | 1.246 | 0.73% | unknown | unknown | derived_cache |
| `idx_entities_qname` | index | unknown | 0.980 | 0.57% | 1028096 | 0 | dictionary |
| `idx_entities_name` | index | unknown | 0.961 | 0.56% | 1015808 | -8192 | proof_optional |
| `idx_qname_prefix_dict_hash` | index | unknown | 0.961 | 0.56% | 3309568 | -2301952 | dictionary |
| `path_dict` | table | 4975 | 0.379 | 0.22% | 397312 | 0 | dictionary |
| `source_content_template` | table | 2718 | 0.191 | 0.11% | 200704 | 0 | template |
...
| `edge_provenance_dict` | table | 1204 | 0.113 | 0.07% | 118784 | 0 | dictionary |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `files_walked` | reported | 0 | pass | Cold proof build files walked. |
| `files_parsed` | reported | 0 | pass | Cold proof build files parsed. |
...
| `duplicate_content_templates` | reported | 0 | pass | Duplicate local analyses skipped by content-template dedupe. |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `cold_proof_build_total_wall_ms` | 60000 | 1941747 | fail | Cold proof build intended target is <=60 seconds. |
| `cold_proof_build_total_profile_ms` | reported | 1941747 | pass | Profile total from compact proof baseline. |
...
| `db_write_time_ms` | reported | 0.000 | pass | Known dominant cold-build stage. |
...
| `integrity_check_time_ms` | reported | 0.000 | pass | Cold proof DB integrity-check duration. |
...
| --- | ---: | ---: | --- | --- | --- |
| `proof-build-only` | 1941747 | 32.362 | fail | true | actual production proof-mode cold build as persisted by the compact gate |
| `proof-build-plus-validation` | 1941747 | 32.362 | fail | false | proof build plus the separately persisted integrity_check duration |
| `proof-build-plus-audit` | 5442112 | 90.702 | reported | false | sequential proof build plus separate audit-sidecar build |
| `proof-build-plus-audit-plus-validation` | 5442112 | 90.702 | reported | false | operator gate bundle, not the production proof-build-only target |
...
| --- | ---: | ---: | --- | --- | --- |
| `actual_proof_db_build_total` | 1941747 | 100.00% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.wall_ms` | This is the persisted cold proof build number. |
| `production_persistence_and_global_reduction_bucket` | 0.000 | 0.00% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.db_write_ms` | This bucket is currently broad: it includes SQLite persistence plus post-local global reduction, PathEvidence refresh, index recreation, ANALYZE/checkpoint work, and transaction commit where applicable. |
| `source_scan_parse_extract_dedupe_reducer_residual` | 1941747 | 100.00% | true | `wall_ms - db_write_ms` | The compact gate did not persist nested cold-build spans for these phases; as a combined residual they are below 5% of proof-build-only wall time. |
| `integrity_check_validation` | 0.000 | 0.00% | false | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.integrity_check_ms` | Integrity checking is separately measured and is not the dominant cause. |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `entity_name_lookup` | 250.000 | 2.467 | 3.609 | 3.609 | pass | Default entity-name lookup must compile and use compact proof entity dictionaries. |
| `symbol_lookup` | 250.000 | 191.002 | 222.471 | 222.471 | pass | Default symbol lookup must resolve object ID, name, and qualified-name paths without audit/debug sidecars. |
| `qname_lookup` | 250.000 | 18.846 | 20.610 | 20.610 | pass | Default qname lookup must use compact qualified-name reconstruction. |
| `text_fts_query` | 500.000 | 0.102 | 0.263 | 0.263 | pass | Proof-mode text query must use compact FTS or fail explicitly. |
| `relation_query_calls` | 500.000 | 235.755 | 262.756 | 262.756 | pass | Bounded CALLS relation lookup must use proof edges and compact compatibility views. |
| `relation_query_reads_writes` | 500.000 | 239.631 | 277.153 | 277.153 | pass | Bounded READS/WRITES lookup must compile against compact proof edges. |
| `path_evidence_lookup` | 500.000 | 2.539 | 4.301 | 4.301 | pass | Stored PathEvidence lookup must use proof-mode materialized lookup tables. |
| `source_snippet_batch_load` | 500.000 | 51.859 | 62.622 | 62.622 | pass | Source snippets must load from source files using proof-edge/source-span rows, not redundant SQLite snippet storage. |
| `context_pack_normal` | 2000 | 130.557 | 146.428 | 146.428 | pass | Normal context_pack must use proof-mode stored PathEvidence first and keep source-span labels. |
| `unresolved_calls_paginated` | 1000 | 90.630 | 102.453 | 102.453 | pass | Unresolved-calls pagination may read the heuristic/debug sidecar by design, but it must remain explicit and bounded. |
...
| --- | ---: | ---: | ---: | --- |
| `proof_db_mib_vs_clean_1_63_gib` | 1633 | 171.281 | -1462 | improved |
...
| `cold_proof_build_ms_vs_compact_baseline` | 1941747 | 1941747 | 0.000 | unchanged |
| `proof_db_mib_vs_compact_baseline` | 320.630 | 171.281 | -149.349 | improved |
| `proof_db_mib_vs_previous_comprehensive` | 320.630 | 171.281 | -149.349 | improved |
