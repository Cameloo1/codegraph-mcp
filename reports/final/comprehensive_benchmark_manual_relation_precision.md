...
| Field | Value |
| --- | --- |
| `artifact_path` | reports\final\artifacts\comprehensive_proof_1778642765060.sqlite |
| `artifact_metadata_path` | reports\final\artifacts\comprehensive_proof_1778642765060.artifact.json |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `proof_db_bytes` | reported | 179531776 | pass | Proof DB family size in bytes. |
| `proof_db_mib` | 250.000 | 171.215 | pass | Proof DB family must be <=250 MiB. |
| `proof_db_mib_stretch` | 150.000 | 171.215 | fail | Stretch target is <=150 MiB. |
...
| `total_artifact_bytes` | reported | 1243164672 | pass | Proof + audit artifact bytes for operator planning. |
...
| `bytes_per_proof_edge` | 120.000 | 5650 | fail | Whole proof DB bytes divided by physical proof-edge rows; this remains a bloat signal. |
...
| `template_edges` | table | 169290 | 16.922 | 9.88% | 17743872 | 0 | template |
...
| `qname_prefix_dict` | table | 48603 | 5.863 | 3.42% | 6148096 | 0 | dictionary |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `cold_proof_build_total_wall_ms` | 60000 | 331868 | fail | Cold proof build intended target is <=60 seconds. |
| `cold_proof_build_total_profile_ms` | reported | 331868 | pass | Profile total from compact proof baseline. |
...
| --- | ---: | ---: | --- | --- | --- |
| `proof-build-only` | 331868 | 5.531 | fail | true | actual production proof-mode cold build as persisted by the compact gate |
| `proof-build-plus-validation` | 331868 | 5.531 | fail | false | proof build plus the separately persisted integrity_check duration |
| `proof-build-plus-audit` | 3832233 | 63.871 | reported | false | sequential proof build plus separate audit-sidecar build |
| `proof-build-plus-audit-plus-validation` | 3832233 | 63.871 | reported | false | operator gate bundle, not the production proof-build-only target |
...
| --- | ---: | ---: | --- | --- | --- |
| `actual_proof_db_build_total` | 331868 | 100.00% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.wall_ms` | This is the persisted cold proof build number. |
...
| `source_scan_parse_extract_dedupe_reducer_residual` | 331868 | 100.00% | true | `wall_ms - db_write_ms` | The compact gate did not persist nested cold-build spans for these phases; as a combined residual they are below 5% of proof-build-only wall time. |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `entity_name_lookup` | 250.000 | 1.847 | 2.660 | 2.660 | pass | Default entity-name lookup must compile and use compact proof entity dictionaries. |
| `symbol_lookup` | 250.000 | 199.356 | 226.865 | 226.865 | pass | Default symbol lookup must resolve object ID, name, and qualified-name paths without audit/debug sidecars. |
| `qname_lookup` | 250.000 | 19.716 | 22.395 | 22.395 | pass | Default qname lookup must use compact qualified-name reconstruction. |
| `text_fts_query` | 500.000 | 0.051 | 0.203 | 0.203 | pass | Proof-mode text query must use compact FTS or fail explicitly. |
| `relation_query_calls` | 500.000 | 244.920 | 254.086 | 254.086 | pass | Bounded CALLS relation lookup must use proof edges and compact compatibility views. |
| `relation_query_reads_writes` | 500.000 | 243.082 | 269.835 | 269.835 | pass | Bounded READS/WRITES lookup must compile against compact proof edges. |
| `path_evidence_lookup` | 500.000 | 2.638 | 3.855 | 3.855 | pass | Stored PathEvidence lookup must use proof-mode materialized lookup tables. |
| `source_snippet_batch_load` | 500.000 | 51.412 | 63.638 | 63.638 | pass | Source snippets must load from source files using proof-edge/source-span rows, not redundant SQLite snippet storage. |
| `context_pack_normal` | 2000 | 132.957 | 149.321 | 149.321 | pass | Normal context_pack must use proof-mode stored PathEvidence first and keep source-span labels. |
| `unresolved_calls_paginated` | 1000 | 94.267 | 106.661 | 106.661 | pass | Unresolved-calls pagination may read the heuristic/debug sidecar by design, but it must remain explicit and bounded. |
...
| --- | ---: | ---: | ---: | --- |
| `proof_db_mib_vs_clean_1_63_gib` | 1633 | 171.215 | -1462 | improved |
...
| `cold_proof_build_ms_vs_compact_baseline` | 331868 | 331868 | 0.000 | unchanged |
| `proof_db_mib_vs_compact_baseline` | 320.630 | 171.215 | -149.415 | improved |
| `proof_db_mib_vs_previous_comprehensive` | 171.215 | 171.215 | 0.000 | unchanged |
