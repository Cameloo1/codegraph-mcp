...
| Field | Value |
| --- | --- |
| `artifact_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_cgc_recovery_20260513.sqlite |
| `artifact_metadata_path` | <REPO_ROOT>/Desktop\development\codegraph-mcp\reports\final\artifacts\comprehensive_proof_cgc_recovery_20260513.artifact.json |
...
| Metric | Target | Observed | Status | Notes |
| --- | --- | --- | --- | --- |
| `cold_proof_build_total_wall_ms` | 60000 | 184297 | fail | Cold proof build intended target is <=60 seconds. |
| `cold_proof_build_total_profile_ms` | reported | 184297 | pass | Profile total from compact proof baseline. |
...
| `db_write_time_ms` | reported | 113085 | pass | Known dominant cold-build stage. |
...
| `integrity_check_time_ms` | reported | 489.146 | pass | Cold proof DB integrity-check duration. |
...
| --- | ---: | ---: | --- | --- | --- |
| `proof-build-only` | 184297 | 3.072 | fail | true | actual production proof-mode cold build as persisted by the compact gate |
| `proof-build-plus-validation` | 184786 | 3.080 | fail | false | proof build plus the separately persisted integrity_check duration |
| `proof-build-plus-audit` | 3684662 | 61.411 | reported | false | sequential proof build plus separate audit-sidecar build |
| `proof-build-plus-audit-plus-validation` | 3685151 | 61.419 | reported | false | operator gate bundle, not the production proof-build-only target |
...
| --- | ---: | ---: | --- | --- | --- |
| `actual_proof_db_build_total` | 184297 | 100.00% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.wall_ms` | This is the persisted cold proof build number. |
| `production_persistence_and_global_reduction_bucket` | 113085 | 61.36% | true | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.db_write_ms` | This bucket is currently broad: it includes SQLite persistence plus post-local global reduction, PathEvidence refresh, index recreation, ANALYZE/checkpoint work, and transaction commit where applicable. |
| `source_scan_parse_extract_dedupe_reducer_residual` | 71212 | 38.64% | true | `wall_ms - db_write_ms` | The compact gate did not persist nested cold-build spans for these phases; as a combined residual they are below 5% of proof-build-only wall time. |
| `integrity_check_validation` | 489.146 | 0.26% | false | `reports/final/compact_proof_db_gate.json.autoresearch.proof_build.integrity_check_ms` | Integrity checking is separately measured and is not the dominant cause. |
...
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `entity_name_lookup` | 250.000 | 1.946 | 2.947 | 2.947 | pass | Default entity-name lookup must compile and use compact proof entity dictionaries. |
| `symbol_lookup` | 250.000 | 191.987 | 226.810 | 226.810 | pass | Default symbol lookup must resolve object ID, name, and qualified-name paths without audit/debug sidecars. |
| `qname_lookup` | 250.000 | 19.159 | 19.924 | 19.924 | pass | Default qname lookup must use compact qualified-name reconstruction. |
| `text_fts_query` | 500.000 | 0.047 | 0.194 | 0.194 | pass | Proof-mode text query must use compact FTS or fail explicitly. |
| `relation_query_calls` | 500.000 | 227.957 | 236.474 | 236.474 | pass | Bounded CALLS relation lookup must use proof edges and compact compatibility views. |
| `relation_query_reads_writes` | 500.000 | 228.492 | 238.205 | 238.205 | pass | Bounded READS/WRITES lookup must compile against compact proof edges. |
| `path_evidence_lookup` | 500.000 | 2.580 | 4.443 | 4.443 | pass | Stored PathEvidence lookup must use proof-mode materialized lookup tables. |
| `source_snippet_batch_load` | 500.000 | 48.898 | 51.370 | 51.370 | pass | Source snippets must load from source files using proof-edge/source-span rows, not redundant SQLite snippet storage. |
| `context_pack_normal` | 2000 | 128.758 | 140.259 | 140.259 | pass | Normal context_pack must use proof-mode stored PathEvidence first and keep source-span labels. |
| `unresolved_calls_paginated` | 1000 | 92.030 | 98.392 | 98.392 | pass | Unresolved-calls pagination may read the heuristic/debug sidecar by design, but it must remain explicit and bounded. |
...
| `cold_proof_build_ms_vs_compact_baseline` | 184297 | 184297 | 0.000 | unchanged |
