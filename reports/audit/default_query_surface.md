...
- Status: **passed**
- DB: `<REPO_ROOT>/Desktop\development\codegraph-mcp\reports\audit\artifacts\default_query_surface_1778633225961.sqlite`
- Iterations: `20`

| Query | Status | p50 ms | p95 ms | p99 ms | rows | target p95 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| `entity_name_lookup` | `pass` | 0.565 | 1.489 | 1.489 | 5 | 250.000 |
| `symbol_lookup` | `pass` | 0.807 | 1.255 | 1.255 | 5 | 250.000 |
| `qname_lookup` | `pass` | 0.660 | 1.431 | 1.431 | 2 | 250.000 |
| `text_fts_query` | `pass` | 0.054 | 0.157 | 0.157 | 2 | 500.000 |
| `relation_query_calls` | `pass` | 0.691 | 1.146 | 1.146 | 11 | 500.000 |
| `relation_query_reads_writes` | `pass` | 0.684 | 1.158 | 1.158 | 24 | 500.000 |
| `path_evidence_lookup` | `pass` | 0.125 | 0.365 | 0.365 | 50 | 500.000 |
| `source_snippet_batch_load` | `pass` | 0.749 | 0.877 | 0.877 | 50 | 500.000 |
| `context_pack_normal` | `pass` | 2.792 | 3.735 | 3.735 | 1 | 2000 |
| `unresolved_calls_paginated` | `pass` | 2.217 | 2.663 | 2.663 | 0 | 1000 |
...
```sql
SELECT oid.value AS id, name.value AS name, qname.value AS qualified_name, path.value AS repo_relative_path FROM entities e JOIN object_id_lookup oid ON oid.id = e.id_key JOIN symbol_dict name ON name.id = e.name_id JOIN qualified_name_lookup qname ON qname.id = e.qualified_name_id JOIN path_dict path ON path.id = e.path_id WHERE name.value = 'user' ORDER BY qname.value, oid.value LIMIT 20
```
...
| id | parent | detail |
| ---: | ---: | --- |
| 2 | 0 | `CO-ROUTINE object_id_lookup` |
| 3 | 2 | `COMPOUND QUERY` |
| 4 | 3 | `LEFT-MOST SUBQUERY` |
| 8 | 4 | `SCAN e USING INDEX idx_entities_qname` |
| 12 | 4 | `SEARCH debug USING INTEGER PRIMARY KEY (rowid=?) LEFT-JOIN` |
| 29 | 3 | `UNION ALL` |
| 31 | 29 | `SCAN debug` |
| 34 | 29 | `CORRELATED SCALAR SUBQUERY 6` |
| 38 | 34 | `SEARCH e USING PRIMARY KEY (id_key=?)` |
| 50 | 3 | `UNION ALL` |
| 52 | 50 | `SCAN history` |
| 55 | 50 | `CORRELATED SCALAR SUBQUERY 3` |
| 59 | 55 | `SEARCH e USING PRIMARY KEY (id_key=?)` |
| 69 | 50 | `CORRELATED SCALAR SUBQUERY 4` |
| 73 | 69 | `SEARCH debug USING INTEGER PRIMARY KEY (rowid=?)` |
| 97 | 0 | `SCAN e USING INDEX idx_entities_qname` |
| 101 | 0 | `SEARCH name USING INTEGER PRIMARY KEY (rowid=?)` |
| 106 | 0 | `SEARCH qname USING INTEGER PRIMARY KEY (rowid=?)` |
| 109 | 0 | `SEARCH path USING INTEGER PRIMARY KEY (rowid=?)` |
| 112 | 0 | `SEARCH prefix USING INTEGER PRIMARY KEY (rowid=?)` |
| 115 | 0 | `SEARCH suffix USING INTEGER PRIMARY KEY (rowid=?)` |
| 120 | 0 | `BLOOM FILTER ON oid (id=?)` |
| 132 | 0 | `SEARCH oid USING AUTOMATIC COVERING INDEX (id=?)` |
| 167 | 0 | `USE TEMP B-TREE FOR ORDER BY` |
...
```sql
SELECT oid.value AS id, name.value AS name, qname.value AS qualified_name, path.value AS repo_relative_path FROM entities e JOIN object_id_lookup oid ON oid.id = e.id_key JOIN symbol_dict name ON name.id = e.name_id JOIN qualified_name_lookup qname ON qname.id = e.qualified_name_id JOIN path_dict path ON path.id = e.path_id WHERE oid.value = 'user' OR name.value = 'user' OR qname.value = 'user' ORDER BY qname.value, oid.value LIMIT 20
```
...
| 73 | 69 | `SEARCH debug USING INTEGER PRIMARY KEY (rowid=?)` |
| 97 | 0 | `SCAN e USING INDEX idx_entities_qname` |
| 101 | 0 | `SEARCH qname USING INTEGER PRIMARY KEY (rowid=?)` |
| 104 | 0 | `SEARCH path USING INTEGER PRIMARY KEY (rowid=?)` |
| 107 | 0 | `SEARCH name USING INTEGER PRIMARY KEY (rowid=?)` |
| 110 | 0 | `SEARCH prefix USING INTEGER PRIMARY KEY (rowid=?)` |
| 113 | 0 | `SEARCH suffix USING INTEGER PRIMARY KEY (rowid=?)` |
| 118 | 0 | `BLOOM FILTER ON oid (id=?)` |
| 130 | 0 | `SEARCH oid USING AUTOMATIC COVERING INDEX (id=?)` |
| 186 | 0 | `USE TEMP B-TREE FOR ORDER BY` |
...
```sql
SELECT oid.value AS id, name.value AS name, qname.value AS qualified_name, path.value AS repo_relative_path FROM entities e JOIN object_id_lookup oid ON oid.id = e.id_key JOIN symbol_dict name ON name.id = e.name_id JOIN qualified_name_lookup qname ON qname.id = e.qualified_name_id JOIN path_dict path ON path.id = e.path_id WHERE qname.value = 'admin_user_middleware_role_separation::repo::src::auth.requireUser.user' ORDER BY oid.value LIMIT 20
```
...
| 110 | 0 | `SEARCH prefix USING INTEGER PRIMARY KEY (rowid=?)` |
| 113 | 0 | `SEARCH suffix USING INTEGER PRIMARY KEY (rowid=?)` |
| 135 | 0 | `BLOOM FILTER ON oid (id=?)` |
| 147 | 0 | `SEARCH oid USING AUTOMATIC COVERING INDEX (id=?)` |
| 182 | 0 | `USE TEMP B-TREE FOR ORDER BY` |
...
```sql
SELECT kind, id, repo_relative_path, line, title, bm25(stage0_fts) AS rank FROM stage0_fts WHERE stage0_fts MATCH '"source_span_exact_callsite"' ORDER BY rank, kind, id LIMIT 20
```
