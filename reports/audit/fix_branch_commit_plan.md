...
- Intended branch: `fix`
- Current `git rev-parse --abbrev-ref HEAD`: `HEAD`
- Commit readiness note: confirm or attach this worktree to `fix` before committing. Do not create the commit from a detached head unless that is intentional and immediately reconciled onto `fix`.
...
```text
fix: centralize DB passport scope lifecycle
...
- Confirm the worktree is attached to `fix`, not detached `HEAD`.
