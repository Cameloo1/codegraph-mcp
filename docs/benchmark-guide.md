...
- [SWE-bench Readiness](swe-bench-readiness.md) tracks current local harness
  readiness, blockers, and what remains before CodeGraph-attributed patch
  outcomes can be claimed.
...
```powershell
.\target\release\codegraph-mcp.exe bench comprehensive --fresh --repo fixtures/smoke/basic_repo --workers 2 --output-dir reports\audit\artifacts\<run_id> --timestamp local --max-db-mib 128 --max-artifacts-mib 250 --min-free-disk-gib 1
```
